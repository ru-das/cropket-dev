...
```
`test:e2e` needs `scripts/.env` and two test OTPs added in the Supabase dashboard (Auth → Phone →
test OTPs): farmer `9090910010`/`910010`, buyer `9090920001`/`920001` - separate from the M5 demo
accounts, so it never touches `demo-reset`'s own data. `tests/e2e/global-setup.ts` gets both ready
...
- [2026-09-17] `supabase functions new <name>` (CLI 2.117.0) now also appends its own `[functions.<name>]` block to `supabase/config.toml` (with `import_map = "./functions/<name>/deno.json"`, pointing at the per-function file the same command scaffolds). Delete that auto-appended block, not just the stray `deno.json` file, before adding the project's real block by hand. (Why: `db push`/`functions deploy` failed with `CliConfigParseError` from a duplicate `[functions.cron-fetch-prices]` table - TOML rejects declaring the same table twice - because both the auto-appended block and the hand-added one were left in the file.)
- [2026-09-23] An outbox handler that reads `db.outbox` back inside its own follow-up query (the "is this id still queued?" check `services/lots.ts`'s `getLot()` uses) must delete its own outbox row itself, before it invalidates any query - call the new `dequeue(id)` (`offline/outbox.ts`), don't wait for `offline/sync.ts`'s `sendOne()` to delete it after the handler returns. (Why: `sendOne()` only deletes *after* the handler resolves, so a handler's own `invalidateQueries()` call - which runs *before* it returns - can still see its own not-yet-deleted row and re-cache the wrong ("still pending") result; `offline/persist.ts`'s 5-minute `staleTime` then keeps that wrong result around, reload or not. Found building 5.2's e2e test: a lot had already synced to `cropket-dev`, but the UI stayed stuck "On phone only" even after a reload.)
- [2026-09-23] A reload/goto in a Playwright test that needs to see a change made on a *different* browser context (buyer paid, driver delivered) must clear Dexie's persisted query cache first (`indexedDB.open("cropket")` → clear the `cache` object store - a `clearPersistedCache()` helper lives in `tests/e2e/core-flow.spec.ts`, copy its shape for a new e2e test rather than guessing a wait). Prefer clicking a real in-app link over a fresh `goto()` wherever the page you're already on has a live Realtime subscription that would refresh it for free (see `core-flow.spec.ts`'s "Offers (N)" step). (Why: `offline/persist.ts`'s `staleTime` (5 min) + `refetchOnWindowFocus: false` mean a plain reload restores the *persisted* snapshot from before the other browser's change and shows it as still fresh, not a fresh fetch - the same root shape as the outbox rule above, but with no in-app fix available, since several money/status screens (SPEC §5.6 `escrow:{dealId}`) don't have a Realtime subscription yet to self-heal it. That gap is real and still open - see 5.2's own handoff note "known gaps" for where to add it.)
