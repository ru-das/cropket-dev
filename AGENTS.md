...
- [2026-09-24] When a fix changes what an already-cached query means (not just its shape), bump `buster` in `app/src/offline/persist.ts`, the same as for a shape change. (Why: after deploying the "My Lots" fix (`listMyLots()` now filters by `farmer_id`), testers who had the app open before the deploy still saw the old wrong list - the service worker updated to the fixed code, but `persistOptions`'s persisted `["lots","mine"]` result from the old code was restored from IndexedDB and, with `staleTime` 5 min and `refetchOnWindowFocus` off, counted as fresh instead of being refetched. A fresh incognito load was correct the whole time, which is what pointed at the persisted cache rather than the deploy.)
- [2026-09-24] The Agmarknet arrivals dashboard (`agmarknet/real.ts` `fetchAgmarknetArrival`) never answers for the same day it's asked about - it always answers for an earlier `reported_date` (checked live: asked for 24-09, got 22-09). `mergePricesWithArrivals`'s same-day match is correct (never pair a wrong day's arrivals with today's price - CLAUDE.md §5 honesty rule), so treat "no same-day match" as the normal case, not a rare edge case: it now returns those as `lateArrivals` for the caller to write onto the historical price row that's already in the DB, instead of a one-off "not a bug" note. (Why: the mandi heatmap was stuck on white/seeded-green pins forever, because every real arrivals figure from the Agmarknet dashboard was silently dropped instead of just occasionally.)
