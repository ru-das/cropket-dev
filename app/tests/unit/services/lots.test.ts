...
// where "My Lots" showed every farmer's listed lots on every account.
// `lots_select_listed` (20260922150000_lots_marketplace.sql) lets any
// signed-in user read a listed/in_mega lot, so RLS alone can't scope "my
// own rows" - the query must filter by farmer_id itself. This locks that
// filter in.
