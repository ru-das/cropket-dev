-- RLS for `grade_results` (SPEC.md §5.6, §5.4). Only the `grade` Edge
-- Function (service role) ever writes this table - `authenticated` has
-- select-only, so this test proves select-your-own plus that insert/update
-- are both rejected for a logged-in farmer. Everything here rolls back.
begin;
select plan(4);

-- two fake auth users + profiles, same pattern as rls_profiles.sql.
